          ...
          <div className="mb-6 flex gap-2">
            {ANIMAL_TYPES.map((t) => {
              ...
              const inactiveMap: Record<string, string> = {
                Cow: "border-transparent text-zinc-500 hover:border-emerald-200 hover:bg-emerald-50",
                Goat: "border-transparent text-zinc-500 hover:border-amber-200 hover:bg-amber-50",
                Hen: "border-transparent text-zinc-500 hover:border-rose-200 hover:bg-rose-50",
                Duck: "border-transparent text-zinc-500 hover:border-sky-200 hover:bg-sky-50",
              };
              return (
                <button key={t} onClick={() => router.push(`/items/manage?type=${t}`)} className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition ${isActive ? colorMap[t] : inactiveMap[t]}`}>
