...
import { AnimalType, useAnimalType } from "@/context/AnimalTypeContext";
...
export default function AnimalPage({ animalType }: AnimalPageProps) {
  const { theme } = useAnimalType();
  const Panel = PANEL_MAP[animalType];
  ...
  return (
    ...
    >
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h2 className={`text-2xl font-bold ${theme.accent}`}>
            {theme.emoji} {animalType} Dashboard
          </h2>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        {SHOW_WEIGHT_GROWTH.includes(animalType) && (
          <WeightGrowthCard />
        )}
      </div>
