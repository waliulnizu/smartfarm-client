    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-emerald-100 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <div className="hidden items-center gap-3 md:flex">
          {user ? (
            <>
              <Link href="/dashboard/cow" className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition">
                Dashboard
              </Link>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 rounded-xl border border-zinc-200 bg-white py-1 shadow-lg">
                    <div className="border-b border-zinc-100 px-4 py-3">
                      ...
                      <span className="mt-1 inline-block rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-medium text-emerald-700">{user.role}</span>
                    </div>
      ...
      {mobileOpen && (
        <div className="border-t border-emerald-100 bg-white px-4 pb-4 md:hidden">
          ...
          {user && (
            <Link href="/dashboard/cow" onClick={() => setMobileOpen(false)} className="mt-1 block rounded-lg bg-emerald-600 px-3 py-2.5 text-center text-sm font-semibold text-white hover:bg-emerald-700">
              Dashboard
            </Link>
          )}
